// Модуль управления бойлером v. 0.2. Релиз от 06.01.2022. 21:20
...
#define Close_Drainage_COLD     20              // ПИН Закрыть слив ХВС из бойлера (аварийный режим)
#define Open_Drainage_COLD      21              // ПИН Открыть слив ХВС из бойлера (аварийный режим)
...
long sec_tapActive = 4500;                      // Определяем время работы кранов в милисекундах
long sec_btnClick = 6000;                       // Спустя это количестве милисекунд обрабатывает нажатие кнопки
...
void loop()
{
  ...
  if (ledEnabled && millis() - time_tapActive <= sec_tapActive && !protect88 && !protect89){
    digitalWrite(LED_PIN, ledEnabled);        // Включаем реле бойлера
    digitalWrite (Open_HOT88, LOW);           // Канал "Открыть ГВС88" - инактивируем
...
// Если Светодиод не горит и не срабатывала защита от протечки - выключаем режим бойлера
  if (!ledEnabled && millis() - time_tapActive <= sec_tapActive && !protect88 && !protect89){
    digitalWrite(LED_PIN, ledEnabled);        // Выключаем реле бойлера
 ...
 if (digitalRead (Dry_Neptun88) == LOW && !protect88) {
    ...
    digitalWrite (Open_COLD88, LOW);          // Канал "Открыть ХВС88" - инактивируем
    digitalWrite (Open_COLD89, LOW);          // Канал "Открыть ХВС89" - инактивируем
    digitalWrite (Close_HOT88, HIGH);          // Канал "Закрыть ГВС88" - активируем. Закрываем горячую воду 88
    digitalWrite (Close_HOT89, HIGH);          // Канал "Закрыть ГВС89" - активируем. Закрываем горячую воду 89
    digitalWrite (Close_COLD88, HIGH);         // Канал "Закрыть ХВС88" - активируем. Закрываем холодную воду 88
    digitalWrite (Close_COLD89, HIGH);         // Канал "Закрыть ХВС89" - активируем. Закрываем холодную воду 89
    digitalWrite (Open_Drainage_HOT, HIGH);    // Канал "Открыть слив ГВС из бойлера" - активируем. Сливаем горячую воду из бойлера
    digitalWrite (Close_Drainage_HOT, LOW);   // Канал "Закрыть слив ГВС из бойлера" - инактивируем.
    digitalWrite (Open_Drainage_COLD, LOW);   // Канал "Открыть слив ХВС из бойлера" - активируем. Сливаем холодную воду из подачи в бойлер
    digitalWrite (Close_Drainage_COLD, LOW);  // Канал "Закрыть слив ХВС из бойлера" - инактивируем
    digitalWrite (Open_Input_Water, LOW);     // Канал "Открыть подачу воды в бойлер" - инактивируем   
    digitalWrite (Close_Input_Water, HIGH);    // Канал "Закрыть подачу воды в бойлер" - активируем. Закрываем подачу воды в бойлер
  }
 ...
 if (digitalRead (Dry_Neptun89) == LOW && !protect89) {
    ...
    digitalWrite (Close_HOT89, HIGH);          // Канал "Закрыть ГВС89" - активируем. Закрываем горячую воду 89
    digitalWrite (Close_COLD88, HIGH);         // Канал "Закрыть ХВС88" - активируем. Закрываем холодную воду 88
    digitalWrite (Close_COLD89, HIGH);         // Канал "Закрыть ХВС89" - активируем. Закрываем голоную воду 89
    digitalWrite (Open_Drainage_HOT, HIGH);    // Канал "Открыть слив ГВС из бойлера" - активируем. Сливаем горячую воду из бойлера
    digitalWrite (Close_Drainage_HOT, LOW);   // Канал "Закрыть слив ГВС из бойлера" - инактивируем.
    digitalWrite (Open_Drainage_COLD, LOW);   // Канал "Открыть слив ХВС из бойлера" - активируем. Сливаем холодную воду из подачи в бойлер
    digitalWrite (Close_Drainage_COLD, LOW);  // Канал "Закрыть слив ХВС из бойлера" - инактивируем
